...
use App\Exports\ExportPocketMoney;
...
class PocketMoneyReportController extends Controller
{
    public function pocketMoneyReport(Request $request) 
    {
        $approveStatus = array(
            '0'=>'Pending',
            '1'=>'Confirm',
        ...
        if ($request['action'] == 'search') {
            ...
            foreach ($result as $res) {
                ...
                $resArr['student_name']   = $res->student_name;
                $resArr['guardian_name']  = $res->guardian_name;
                $resArr['created_at']     = date('Y-m-d',strtotime($res->attendance_at));
                $Data[] = $resArr;
            }
    
            return Excel::download(new ExportPocketMoney($Data), 'pocket_money_export.csv');
        } else {
            request()->merge([
        ...
        $res->select('guardian_pocket_money.*','student_info.name as student_name',
            'student_guardian.name as guardian_name');
        $res = $res->paginate(20);
        return view('admin.report.pocketmoneyreport',[
            'list_result' => $res,
    ...
    /*public function pocketMoneyApprove(Request $request)
    {
        ...
        try {
            ...
            if ($wallet_id) {
                ...
                if ($result) {              
                    DB::commit();
                    return redirect(url('admin/reporting/pocket_money_report'))->with('success', 'Successfully Approve!');
                }
            }
