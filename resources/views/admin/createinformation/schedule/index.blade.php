...
<section class="card">
    <div class="card-body">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-2">
                <a class="btn btn-sm btn-primary" href="{{route('schedule.create')}}" id="form-header-btn"><span class="bi bi-plus"></span> Create</a>
            </div>
        </div>
        <br />
                ...
                <div class="form-group col-md-3">
					...
					<div class="col-sm-10">
                        <select class="form-select" id="schedule_classid" name="schedule_classid">
                            ...
                            <option value="{{$key}}" 
                            @if (request()->input('schedule_classid') == $key)
                                selected
                            @endif
                            >{{$value}}</option>
                            @endforeach
				...
				<div class="form-group col-md-3">
					...
					<div class="col-sm-10">
                        <select class="form-select" id="schedule_teacher" name="schedule_teacher">
                            ...
                            <option value="{{$key}}" 
                            @if (request()->input('schedule_teacher') == $key)
                                selected
                            @endif
                            >{{$value}}</option>
                            @endforeach
				...
				<div class="form-group col-md-3">
					...
					<div class="col-sm-10">
                        <select class="form-select" id="schedule_weekday" name="schedule_weekday">
                            ...
                            <option value="{{$key}}" 
                            @if (request()->input('schedule_weekday') == $key)
                                selected
                            @endif
                            >{{$value}}</option>
                            @endforeach
                    ...
                    @if(!empty($list_result) && $list_result->count())
                        ...
                        <tr>
                            ...
                            <td class="center">
                                ...
                                <form method="post" action="{{route('schedule.destroy',$res->id)}}" style="display: inline;">
                                    @csrf
                                    {{ method_field('DELETE') }}
                                    <button type="submit" value="delete" class="btn m-1 p-0 border-0">
