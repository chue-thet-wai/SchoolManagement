    ...
    <div class="card-body">
        ...
        <div class="row g-4">
            <div class="col-md-10 content-title">
                <h4><b>Exam Rules List</b></h4>
            </div>
            <div class="col-md-2">
                <a class="btn btn-sm btn-primary" href="{{url('admin/exam_rules/create')}}" id="form-header-btn"><span class="bi bi-plus"></span> Create</a>
            </div>
        </div>
        <br />
        <form class="row g-4" method="POST" href="{{ url('admin/exam_rules/list') }}" enctype="multipart/form-data">
			@csrf
			<div class='row g-4'>
				<div class="form-group col-md-3">
					<label for="examrules_title"><b>Title</b></label>
					<div class="col-sm-10">
						<input type="text" name="examrules_title" class="form-control" value="{{ request()->input('examrules_title') }}">
					</div>
				</div>
            </div>		
			<div class='row p-3'>
            ...
            <table cellpadding="0" cellspacing="0" class="datatable table table-striped table-bordered">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Title</th>
                        <th>Grade Range(From)</th>
                        <th>Grade Range(To)</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    @if(!empty($list_result) && $list_result->count())
                        ...
                        <tr>
                            ...
                            <td>{{ $res->title }}</td>
                            <td>{{ $res->mark_range_from }}</td>
                            <td>{{ $res->mark_range_to }}</td>
                            <td class="center">
                                <a href="{{ url('admin/exam_rules/edit/'.$res->id) }}">
                                ...
                                <form method="post" action="{{ url('admin/exam_rules/delete/'.$res->id) }}" style="display: inline;">
                                    @csrf
                                    {{ method_field('DELETE') }}
